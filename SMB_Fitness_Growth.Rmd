...
smbfit_age_data <- smbfit_raw_data[,-c(1:4,6:11,13:17,19:23,25:31:33,35:41)]
...
```
## Growth for the Global dataset
```{r}
## Fit one global model to the entire set of samples to get overall growth between the two rivers.
...
f_starts_global <- vbStarts(tl_alive ~ consensus_age, data = smbfit_age_data_clean_noNA)

#Fit von Bert function
fit_global <- nls(tl_alive ~ vb_function(consensus_age,Linf,K,t0), data = smbfit_age_data_clean_noNA, start = f_starts_global)
...
global_pred <- predict(fit_global,data.frame(consensus_age=2:8))

#write functions to bootstrap over individual ages
pred_2_function_global <- function(x) predict(x,data.frame(consensus_age=ages))
ages=2:8
pred_2_function_global(fit_global)


ages <- seq(-1,15,by=0.2)
global_indiv_boot <- Boot(fit_global, f = pred_2_function_global) #998 out of 999
...
vonbert_global <- ggplot() + 
  ...
  geom_point(data = smbfit_age_data_clean_actual, aes(x = consensus_age, y = tl_alive, fill = "grey"), show.legend = F, size=3, pch=21, position = position_jitter(width = 0.2)) + 
...
vonbert_admix_status <- ggplot() + 
  ...
  geom_point(data=smbfit_age_data_clean_actual,aes(x = consensus_age,y = tl_alive, fill = hyb_status), show.legend = T, size=3, pch=21, position = position_jitter(width = 0.2)) + 
...
vonbert_sex <- ggplot() + 
  ...
  geom_point(data=smbfit_age_data_clean_actual,aes(x = consensus_age,y = tl_alive, fill = sex), show.legend = T, size=3, pch=21, position = position_jitter(width = 0.2)) + 
...
vonbert_rivers <- ggplot() + 
  geom_ribbon(data=preds1_all_rivers, aes(x = age, ymin = LCI, ymax = UCI,fill = river, alpha=0.01,), show.legend = F) +
  geom_point(data=smbfit_age_data_clean_actual, aes(x = consensus_age, y = tl_alive, fill = river), show.legend = T, size=3, pch=21, position = position_jitter(width = 0.2)) + 
...
vonbert_rivers_elk <- ggplot() + 
  geom_ribbon(data=preds1_all_rivers_elk, aes(x = age, ymin = LCI, ymax = UCI,fill = river, alpha=0.01,), show.legend = F) +
  geom_point(data=smbfit_age_data_clean_actual_elk, aes(x = consensus_age, y = tl_alive, fill = river), show.legend = T, size=3, pch=21, position = position_jitter(width = 0.2)) + 
  geom_line(data=preds1_all_rivers_elk, aes(x=age, y=fit, linetype = river)) +
  theme_set(theme_cowplot(12)) +
  scale_fill_manual(values=c("orchid1")) +
  scale_y_continuous(name="Total Length (mm)",limits=c(0,520),expand=c(0,0)) +
  scale_x_continuous(name="Age (years)",expand=c(0,0),limits=c(0,15),breaks=seq(0,15,1)) +
  labs(fill = "River", linetype = "River") +
  theme(axis.title = element_text(size = 20)) + 
...
pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Fitness/visualization/growth_figures/all_von_berts.pdf", width = 15, height = 15)

plot_grid(vonbert_global,
          vonbert_rivers,
          vonbert_sex,
...
## Do some plotting for public dissertation defense
preds1_all_rivers_elk <- preds1_all_rivers %>%
  filter(river == "ELK")

preds1_all_rivers_bsug <- preds1_all_rivers %>%
  filter(river == "BSUG")
...
smbfit_age_data_clean_actual_bsug <- smbfit_age_data_clean_actual %>%
   filter(river == "BSUG")
pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Fitness/visualization/growth_figures/von_bert_elk_river.pdf", width = 8, height = 6)

ggplot() + 
  geom_ribbon(data=preds1_all_rivers_elk, aes(x = age, ymin = LCI, ymax = UCI,fill = river, alpha=0.01,), show.legend = F) +
  geom_point(data=smbfit_age_data_clean_actual_elk, aes(x = consensus_age, y = tl_alive, fill = river), show.legend = F, size=3, pch=21, position = position_jitter(width = 0.2)) +
  theme_set(theme_cowplot(12)) +
  geom_line(data=preds1_all_rivers_elk, aes(x=age, y=fit, linetype = "longdash"), linetype = "dashed", show.legend = F) +
  scale_fill_manual(values=c("sienna4")) +
  scale_y_continuous(name="Total Length (mm)",limits=c(0,520),expand=c(0,0)) +
  scale_x_continuous(name="Age (years)",expand=c(0,0),limits=c(0,15),breaks=seq(0,15,1)) +
  labs(fill = "River", linetype = "River") +
  theme(axis.title = element_text(size = 20)) + 
...
elk_preds1_all_smb_neosho_northern <- elk_preds1_all_smb %>%
  filter(hyb_status ==c("Neosho","Northern"))

smbfit_age_data_clean_actual_northern_neosho <- smbfit_age_data_clean_actual %>%
  filter(hyb_status ==c("Neosho","Northern"))

pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Fitness/visualization/growth_figures/von_bert_gen_elk_neosho_northern.pdf", width = 6, height = 6)

ggplot() + 
  geom_ribbon(data=elk_preds1_all_smb_neosho_northern, aes(x = age, ymin = LCI, ymax = UCI, fill = hyb_status, alpha=0.01,), show.legend = F) +
  geom_point(data=smbfit_age_data_clean_actual_northern_neosho,aes(x = consensus_age,y = tl_alive, fill = hyb_status), show.legend = F, size=3, pch=21, position = position_jitter(width = 0.3)) + 
  geom_line(data=elk_preds1_all_smb_neosho_northern, aes(x=age, y=fit, linetype = hyb_status), show.legend = F) +
  theme_set(theme_cowplot(12)) +
  theme(strip.background = element_rect(fill = "white")) +
  theme(strip.text = element_text(size = 15)) +
  theme(panel.background = element_rect(color = "black", fill = "white", size = 1, linetype = "solid")) +
