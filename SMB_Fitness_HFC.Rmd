...
global_het_fitness_by_hybstatus_plot <- ggplot(global_het_fitness, aes(x = het,y = condition, fill = hyb_status)) + 
  geom_point(size = 2.5, color = "black", pch = 21, show.legend = T) +
  geom_smooth(data = global_het_fitness, aes(x = het, y = condition, color = hyb_status), show.legend = T, method = "lm", se=F) +
  theme_set(theme_cowplot(12)) +
...
bsug_het_fitness_by_hybstatus_plot <- ggplot(bsug_het_fitness, aes(x = het, y = condition, fill = hyb_status)) + 
  geom_point(size = 2.5, color = "black", pch = 21, show.legend = F) +
  geom_smooth(data = bsug_het_fitness, aes(x = het, y = condition, color = hyb_status), show.legend = F, method = "lm", se=F) +
  theme_set(theme_cowplot(12)) +
  labs(x = "Multilocus heterozygosity", y = "Body condition (g/cm3)") +
  theme(axis.title = element_text(size = 20)) +
  theme(axis.text = element_text(size = 15)) +
...
global_het_fitness_admix_lm <- lm(condition ~ het*hyb_status, global_het_fitness_admix)
summary(global_het_fitness_admix_lm)

global_het_fitness_northern_lm <- lm(condition ~ het, global_het_fitness_northern)
summary(global_het_fitness_northern_lm)

global_het_fitness_neosho_lm <- lm(condition ~ het, global_het_fitness_neosho)
summary(global_het_fitness_neosho_lm)

bsug_het_fitness_admix_lm <- lm(condition ~ het, bsug_het_fitness_admix)
summary(bsug_het_fitness_admix_lm)

bsug_het_fitness_northern_lm <- lm(condition ~ het, bsug_het_fitness_northern)
summary(bsug_het_fitness_northern_lm)

bsug_het_fitness_neosho_lm <- lm(condition ~ het, bsug_het_fitness_neosho)
summary(bsug_het_fitness_neosho_lm)


elk_het_fitness_admix_lm <- lm(condition ~ het, elk_het_fitness_admix)
summary(elk_het_fitness_admix_lm)

elk_het_fitness_northern_lm <- lm(condition ~ het, elk_het_fitness_northern)
summary(elk_het_fitness_northern_lm)

elk_het_fitness_neosho_lm <- lm(condition ~ het, elk_het_fitness_neosho)
summary(elk_het_fitness_neosho_lm)
