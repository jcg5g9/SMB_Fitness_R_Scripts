...
smbfit_num_per_size_by_river_by_sex$river_sex_names <- c(rep(c("Big Sugar Creek Female","Big Sugar Creek Male", "Elk River Female","Elk River Male"), times = 8))
...
smb_fit_size_dist_river <- ggplot(smbfit_num_per_size_by_river, aes(x = size_bins, y = n, fill = river_name)) + 
...
smb_fit_size_dist_sex <- ggplot(smbfit_num_per_size_by_sex, aes(x = size_bins, y = n, fill = sex_name)) + 
  ...
  scale_fill_manual(values = c("firebrick3","deepskyblue")) +
  theme(legend.title = element_text(size = 15, face = "bold")) +
  theme(legend.position = c(0.7,0.8)) +
